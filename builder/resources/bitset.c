...
inline void bitset_copy(bitset_elem_t const* src, bitset_elem_t* dst) {
...
inline bool bitset_op_and(bitset_elem_t* a, bitset_elem_t const* b) {
...
inline bool bitset_op_or(bitset_elem_t* a, bitset_elem_t const* b) {
  bitset_elem_t nonzero = 0;
	for (size_t n = 0; n < BITSET_ELEMS_LENGTH; n++) {
		nonzero |= (a[n] |= b[n]);
	}
...
inline size_t bitset_collect_results(bitset_elem_t const* bitset, doc_id_t* append_results_to, size_t max_results) {
