...
const makeKvRequest = path => new Promise((resolve, reject) => request(`http://localhost:${KV_PORT}${path}`, {encoding: null}, (err, res, body) => {
  ...
  if (res.statusCode !== 200) return reject(new Error(`Bad status from KV server: ${res.statusCode}`));
  return resolve(body);
}));
...
global.KV = {
  async get(key, format) {
    let value;
    if (key === 'default') {
      value = await makeKvRequest(`/package/default`);
    } else if (key.startsWith('doc_')) {
      value = await makeKvRequest(`/package/doc/${key.slice(4)}`);
    } else if (key.startsWith('normal_terms_')) {
      value = await makeKvRequest(`/package/normal_terms/${key.slice(13)}`);
    } else if (key.startsWith('popular_terms_')) {
      value = await makeKvRequest(`/package/popular_terms/${key.slice(14)}`);
    } else {
    ...
    case 'json': return JSON.parse(value.toString());
    case 'arrayBuffer': return value.buffer;
    default: throw new Error(`Unimplemented format: ${format}`);
    }
  },
};
...
global.self = {
  addEventListener(eventName, handler) {
    switch (eventName) {
    case 'fetch': onFetch = handler; break;
    default: throw new TypeError(`Unknown event name: ${eventName}`);
    }
  },
};
